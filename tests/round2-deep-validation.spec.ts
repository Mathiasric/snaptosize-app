...
function createTinyJpeg(filePath: string) {
...
test.describe("Image upload + quality warning", () => {
  const tmpImage = path.join(__dirname, "test-tiny.jpg");
  test.beforeAll(() => createTinyJpeg(tmpImage));
  test.afterAll(() => { if (fs.existsSync(tmpImage)) fs.unlinkSync(tmpImage); });

  test("packs: small image + pack → warning", async ({ page }) => {
    await page.goto(`${BASE}/app/packs`);
    await page.locator('input[type="file"]').setInputFiles(tmpImage);
    await page.waitForTimeout(500);
    const btn = page.locator('button:has-text("2×3")').first();
    if (await btn.isVisible()) await btn.click();
    await expect(page.locator("text=Low resolution")).toBeVisible({ timeout: 3000 });
  });

  test("quick-export: small image → warning", async ({ page }) => {
